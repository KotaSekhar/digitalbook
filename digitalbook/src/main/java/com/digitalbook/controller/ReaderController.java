...
public class ReaderController {
	...
	@DeleteMapping(value = "readers/delete/books/{bookId}/{readerId}")
	public ResponseEntity<String> deleteBook(@RequestHeader("Authorization") String token,@PathVariable int bookId, @PathVariable int readerId) {
		...
		String getBookByPaymentId =null;
		if(userRole.equals("ROLE_READER")) {
			getBookByPaymentId = readerBookService.deleteBook(bookId, readerId);
			response = new ResponseEntity<String>(getBookByPaymentId, HttpStatus.OK);
		}else {
			response = new ResponseEntity<String>(new JSONObject().put("response", "Invalid User").toString(),HttpStatus.NOT_FOUND);
